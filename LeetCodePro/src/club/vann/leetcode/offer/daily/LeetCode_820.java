 ...
 * <p>难度：Midum</p>
 * <p>题目：单词压缩</p>
 * <p>描述：给定一个单词列表，我们将这个列表编码成一个索引字符串 S 与一个索引列表 A。
 *
...
public class LeetCode_820 {
    public static void main(String[] args) {
        String[] words = null;
        int result = 0;
        LeetCode_820 leetCode = new LeetCode_820();

        words = new String[]{"time", "me", "bell"};
        result = leetCode.minimumLengthEncoding(words);
        System.out.println("Result[10] : " + result);

        words = new String[]{"time", "world", "bell"};
        result = leetCode.minimumLengthEncoding(words);
        System.out.println("Result[16] : " + result);

        words = new String[]{"time"};
        result = leetCode.minimumLengthEncoding(words);
        System.out.println("Result[5] : " + result);

        words = new String[]{"time", "time"};
        result = leetCode.minimumLengthEncoding(words);
        System.out.println("Result[5] : " + result);

        words = TestCase.words;
        result = leetCode.minimumLengthEncoding(words);
        System.out.println("Result[13956] : " + result);
    }
    ...
    private int minimumLengthEncoding(String[] words) {
        ...
        // 标志当前已经记录的索引
        int point = 0;
        outer:for(int n = 0; n < len; n ++) {
            String str = words[n];
            for(int m = 0; m <= point; m ++) {
                if(words[m].endsWith(str)) {
                    continue outer;
                } else if(str.endsWith(words[m])) {
                    words[m] = str;
                    continue outer;
                }
            }
